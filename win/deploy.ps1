...
if ($configuration -eq "Release") {
  ...
  if ($nuspecFile) {
    ...
    rm    $nugetDir\*.nupkg -ErrorAction SilentlyContinue
    nuget pack $nuspecFile.FullName -OutputDirectory $nugetDir
    nuget push $nugetDir\*.nupkg -Source GitHub
  } else {
